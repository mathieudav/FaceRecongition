...
def split_array(array,nb_elt):
    n=len(array)
    nb_split=int(n/nb_elt+0.5)
   
    return [array[i*nb_elt:(i+1)*nb_elt] if i<nb_split-1 else array[i*nb_elt:] for i in range(nb_split)]
...
for batch in split_array(training_data_set_tracks,number_of_batch):
    training_set=[]
    training_set=list(map(load_preprocess_data,batch))
    print(i)
    i-=len(batch)
    l=np.array(training_set).transpose()
    X_train_tracks=vgg_test.get_img_features(l[0])
    y_train_tracks=l[1]
...
for batch in split_array(training_data_set_people,number_of_batch):
    ...
    i-=len(batch)
    l=np.array(training_set).transpose()
    X_train_people=vgg_test.get_img_features(l[0])
    y_train_people=l[1]
...
for batch in split_array(test_data_set,number_of_batch):
    test_set=[]
    test_set=list(map(load_preprocess_data,batch))
    print(i)
    i-=len(batch)
